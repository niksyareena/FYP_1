"""
Test Format Correction Module with Breast Cancer Dataset
"""
...
def main():
    print("\n" + "="*70)
    print("FORMAT CORRECTION TEST - BREAST CANCER DATASET".center(70))
    print("="*70 + "\n")
    ...
    config = {
        ...
        'standardize_dates': False,  #Breast Cancer dataset has no date columns
        'correct_types': True,
        'string_case': 'lower'
    }
    ...
    print("\n" + "="*70)
    output_path = 'data/output/breast_cancer_corrected.csv'
    df_corrected.to_csv(output_path, index=False)
    print(f"✓ Corrected dataset saved to: {output_path}")
    
    #save corrections log
    corrector.save_corrections_log('data/output/breast_cancer_format_corrections.json')
    print("="*70)
