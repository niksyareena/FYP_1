"""
Test Format Correction Module with Adult Income Dataset
"""
...
def main():
    print("\n" + "="*70)
    print("FORMAT CORRECTION TEST - ADULT INCOME DATASET".center(70))
    print("="*70 + "\n")
    ...
    config = {
        ...
        'standardize_dates': False,  #Adult dataset has no date columns
        'correct_types': True,
        'string_case': 'lower'
    }
    ...
    print("\n" + "="*70)
    output_path = 'data/output/adult_corrected.csv'
    df_corrected.to_csv(output_path, index=False)
    print(f"✓ Corrected dataset saved to: {output_path}")
    
    #save corrections log
    corrector.save_corrections_log('data/output/adult_format_corrections.json')
    print("="*70)
