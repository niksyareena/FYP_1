...
def load_test_data():
    ...
    df = pd.read_csv('data/output/test_duplicates_injected_bank.csv')
...
def test_fuzzy_duplicate_detection(df, ground_truth, threshold=0.75):
    ...
    if true_positives:
        ...
        for idx1, idx2 in sorted(true_positives)[:5]:
            ...
            cols_to_show = ['job', 'education', 'marital', 'contact']
    ...
    if false_positives:
        ...
        for idx1, idx2 in sorted(false_positives)[:5]:
            ...
            row1 = df.iloc[idx1][['job', 'education', 'marital']].to_dict()
            row2 = df.iloc[idx2][['job', 'education', 'marital']].to_dict()
            print(f"     {row1}")
            print(f"     {row2}")
...
def main():
    ...
    print(f"\n{'='*70}")
    print("DUPLICATE DETECTION ALGORITHM TEST - BANK MARKETING")
    print(f"{'='*70}\n")
